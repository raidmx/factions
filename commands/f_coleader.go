...
type FColeaderCmd struct {
	Coleader cmd.SubCommand `cmd:"coleader"`

	Member string `cmd:"member"`
}

func (c FColeaderCmd) Run(src cmd.Source, o *cmd.Output) {
	...
	newRank := factions.CoLeader

	// check if has permission
	if !config.RankHasPermission(rank, "coleader") {
		mustBeRank := config.RankWithNativePermission("coleader")
	...
	// check limits
	coMax := int(config.GetFactionConfig[float64]("number_of_coleaders"))
	if len(faction.MembersWithRank(newRank))+1 == coMax {
		p.Message(config.Message("max_coleaders", fmt.Sprint(coMax)))
		return
	}
