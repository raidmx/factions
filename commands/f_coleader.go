...
func (c FColeaderCmd) Run(src cmd.Source, o *cmd.Output) {
	...
	if !utils.RankHasPermission(rank, "coleader") {
		mustBeRank := utils.RankWithNativePermission("coleader")
	...
	// check limits
	coMax := int(utils.GetFactionConfig[float64]("number_of_coleaders"))
	if len(faction.MembersWithRank(newRank))+1 == coMax {
		p.Message(utils.Message("max_coleaders", fmt.Sprint(coMax)))
		return
	}
